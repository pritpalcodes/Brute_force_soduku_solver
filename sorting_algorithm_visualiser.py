...
# Function to update the bar plot visualization and display time complexity & comparisons
def update_plot(array, colors, title, comparisons, time_complexity):
    plt.clf()  # Clear the figure
    plt.bar(range(len(array)), array, color=colors)
    plt.title(title)
    plt.text(len(array) - 1, max(array) - 1, f"Time Complexity: {time_complexity}", fontsize=10, color='red', ha='right')
    plt.text(0, -5, f"Comparisons: {comparisons}", fontsize=12, color='blue', ha='left')
...
def bubble_sort_visual(array):
    ...
    for i in range(n):
        for j in range(0, n-i-1):
            ...
            update_plot(array, colors, "Bubble Sort", comparisons, "O(n^2)")
    return array
...
def insertion_sort_visual(array):
    ...
    for i in range(1, n):
        ...
        while j >= 0 and key < array[j]:
            ...
            update_plot(array, colors, "Insertion Sort", comparisons, "O(n^2)")
        comparisons += 1  # For the last comparison when key is not less than array[j]
        array[j + 1] = key
    return array

# Merge Sort Visualization
def merge_sort_visual(array):
    comparisons = [0]  # Using a list to allow update within nested function
    ...
    def merge(array, left, mid, right):
        ...
        while i < len(L) and j < len(R):
            ...
            update_plot(array, colors, "Merge Sort", comparisons[0], "O(n log n)")
            k += 1
...
def quick_sort_visual(array):
    ...
    def partition(array, low, high):
        ...
        for j in range(low, high):
            ...
            update_plot(array, colors, "Quick Sort", comparisons[0], "O(n log n)")
        array[i+1], array[high] = array[high], array[i+1]
        update_plot(array, ['green' if x == i+1 or x == high else 'blue' for x in range(len(array))], "Quick Sort", comparisons[0], "O(n log n)")
        return i + 1
